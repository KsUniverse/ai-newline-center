...
- **补充要求**: 设计同域新功能时，优先判断能否复用既有模型、查询构建、组件骨架、交互模式，不能默认新开一套平行实现
...
> 所有角色在需要向用户提问或确认时，必须使用**选择题格式**，减少用户输入负担。
...
用户直接回复字母（如 `A` 或 `A B C`）即可，无需完整描述。

### 适用场景

| 场景 | 角色 | 示例 |
|------|------|------|
| PRD 未明确的边界情况 | PM | 禁用账号时，子账号如何处理？A. 同时禁用 B. 保持不变 |
| 功能点拆分决策 | PM | 是否拆为两个迭代？A. 同一迭代 B. 拆为两个 |
| 技术方案选型 | 架构师 | 数据加载方式？A. 服务端渲染 B. 客户端请求 |
| 评审优先级确认 | 评审 | 此问题是否阻塞发布？A. 阻塞（Critical）B. 不阻塞（Medium）|
...
Step 2: 检查 — 以下文档是否需要新增或更新？
    ...
    - 目录文档: docs/architecture/project-structure.md（若稳定目录或命名发生变化）
    ↓
...
| 后端 | 实现中是否发现可复用的抽象模式？是否出现复制式实现苗头？架构文档和 coding-standards 是否需要补充？ |
...
- **角色**: 产品经理 (PM)
- **输入**: 用户意图 + PRD.md（**完整阅读**） + ROADMAP.md + 已完成迭代的 requirements.md
- **动作**:
  1. **完整阅读** `docs/product/PRD.md`（不可只读对应模块，必须通读全文以理解跨模块依赖和术语体系）
  2. 阅读 ROADMAP.md 确定当前里程碑和下一个待规划迭代
  3. 阅读已完成迭代的 requirements.md 了解已实现功能
  4. 与用户确认本次迭代的功能点范围（1-3 个功能点）
  5. **深度业务对接**: 对每个功能点进行细粒度业务讨论，特别是：
     - 涉及外部服务/API 对接的功能：必须向用户确认对接方式、接口格式、认证方式、错误处理策略
     - 涉及定时任务/后台任务的功能：必须确认触发条件、执行频率、数据同步策略、失败重试机制
     - 涉及数据流转的功能：必须确认数据来源、存储方式、更新策略、一致性要求
  6. 确认每个功能点的细节补充（如有 PRD 未覆盖的内容）
  7. 产出 `docs/product/versions/vX.Y.Z/requirements.md`（必须包含外部服务对接细节）
  8. 更新 ROADMAP.md 中对应迭代的状态
- **检查点**: 用户确认需求文档无误
- **自省**: 回顾 PRD.md 是否有需要补充的模糊需求；ROADMAP 状态是否已更新；外部服务对接细节是否已充分记录。提议修改 → 用户确认
...
- **动作**:
  1. 阅读需求文档
  2. 对照现有架构评估影响范围
  3. **深度技术对接（出方案前必做）**: 在产出技术设计之前，必须就以下内容与用户逐一确认：
     - **领域模型**: 每个新增/修改的数据模型的字段含义、业务规则、状态流转
     - **外部服务集成**: 每个涉及外部 API 的功能点，必须确认：调用方式（REST/SDK/消息队列）、请求响应格式、认证方式、超时重试策略、mock 与真实环境切换方案
     - **业务逻辑细节**: 数据校验规则、权限控制粒度、并发处理策略、数据一致性保证
     - **交互流程**: 复杂业务的完整交互链路（前端操作 → API 调用 → 后台处理 → 状态更新 → 前端反馈）
  4. 设计数据模型变更、API 契约、组件树
     - 对同领域新功能，先判断是否可在既有抽象上扩展；若可通过 `type`、权限、配置或少量差异完成，优先复用，不默认新增平行结构
  5. 拆分前后端开发任务 (tasks-backend.md, tasks-frontend.md)
  6. 若需架构变更，标注 `[ARCH-CHANGE]` 并更新相关架构文档
  7. 产出 `technical-design.md`（必须包含外部服务集成方案的完整细节）
- **检查点**: 用户确认技术方案
- **自省**: 检查 architecture/* 和 standards/* 是否与本版本设计矛盾；是否引入新模式需记录；外部服务集成方案是否完整落库。提议修改 → 用户确认
...
- **实现原则**:
  - 优先抽共享 `where/include/helper/mapper`，不要为同一领域复制一整套近似 Service / Repository
  - 若实现中沉淀出稳定抽象，应视为全局模式候选，进入自省与文档回写流程
- **检查点**: 用户确认后端开发完成
...
- **实现原则**:
  - 若差异只在文案、接口、Badge、权限展示，优先抽共享卡片、Drawer、列表骨架或配置项
  - 禁止“复制一份 accounts 再改名”为 benchmark 或其他同域页面
- **检查点**: 用户确认前端开发完成
...
- **输入**: 前后端代码 + technical-design.md + tasks-frontend.md（了解 [INTEGRATE] 标注上下文）
- **动作**:
...
2. 是否有值得记录到全局文档的通用经验？
3. 是否沉淀出了可复用的稳定抽象（后端 where/include 构建、前端共享骨架、统一交互模式等）？
4. 若有，在任务文件末尾以 `## 自省报告` 写下发现和修改建议，等待用户确认后由架构师执行修改。
```
...
> Bug 修复独立于版本迭代流程。发现 Bug 时立即建立独立分支处理，不合并到迭代 Phase 中。

### 触发场景

| 场景 | 说明 |
|------|------|
| **迭代中发现** | Phase 3-7 期间测试/评审发现当次迭代引入的 Bug |
| **生产 Bug** | Release 后用户发现的线上问题，在 `main` 分支修复 |
| **历史遗留** | 旧版本遗留问题，在当前活跃分支修复 |
...
创建 Bug 记录（bugs.md）
    ↓
创建 bug/BUG-XXX-描述 分支（从受影响分支 checkout）
    ↓
确定责任角色（后端问题 → 后端角色，前端问题 → 前端角色）
    ↓
角色修复代码
    ↓
用户直接测试验证
    ↓ ✅ 通过
合并回受影响分支 → 更新 bugs.md 状态为 Closed
    ↓ ❌ 未通过
继续修复 → 重新测试
```
...
每次迭代目录下维护一个 `bugs.md`，记录该迭代发现的所有 Bug：

```
docs/product/versions/vX.Y.Z/bugs.md
```
...
- **状态**: Open / In Fix / Closed
- **分支**: bug/BUG-001-描述
- **发现时机**: Phase 6 评审 / Phase 7 测试 / 生产反馈
- **描述**: 详细描述问题现象
- **复现步骤**:
  1. 步骤一
  2. 步骤二
...
- **根本原因**: 修复后填写
- **修复说明**: 修复后填写
- **验证结果**: 用户测试通过后填写（通过/失败 + 日期）
```

### 分支命名

```
bug/BUG-001-dashboard-404        # 生产 Bug，从 main checkout
bug/BUG-002-card-invisible-dark   # 迭代中 Bug，从 feature/vX.Y.Z checkout
```

### 合并策略

| Bug 来源 | checkout 基础 | 合并目标 |
|----------|-------------|---------|
| 生产 Bug | `main` | `main` |
| 迭代中 Bug | `feature/vX.Y.Z` | `feature/vX.Y.Z` |

合并后由编排者在 `bugs.md` 更新状态为 Closed。
