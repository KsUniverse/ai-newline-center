...
- [ ] 运行 `pnpm db:generate`，更新 Prisma Client

### T-BE-002: bullmq.ts — RewriteJobData 扩展

- [ ] `src/lib/bullmq.ts`：`RewriteJobData` 新增字段 `mode: "workspace" | "direct"`
- [ ] `src/lib/bullmq.ts`：`RewriteJobData.workspaceId` 改为可选（direct 模式无 workspace）
...
- [ ] 新增 `createDirect(data: CreateDirectRewriteData)` 方法（创建 mode=DIRECT 的 Rewrite）
- [ ] 新增 `updateDirectTaskContext(id, data, db)` 或等价方法（更新 DIRECT 任务的 `topic` / `targetAccountId`）
- [ ] 新增 `findByIdAndUser(id: string, userId: string, organizationId: string, mode?: "DIRECT" | "WORKSPACE")` 方法（个人数据隔离查询）
- [ ] DTO 转换兼容 `workspaceId: null`、`mode`、`topic`

### T-BE-005: Service 层 — rewrite.service.ts

- [ ] 新增 `generateDirect(input: DirectGenerateRewriteInput, caller: SessionUser)` 方法：
...
- [ ] 新增 `getDirectRewrite(rewriteId: string, caller: SessionUser)` 方法：
  1. 调用 `findByIdAndUser(..., mode="DIRECT")`，未找到时抛 `REWRITE_NOT_FOUND` 404
- [ ] 新增 `saveDirectVersionEdit(rewriteId: string, versionId: string, input: SaveRewriteEditInput, caller: SessionUser)` 方法

### T-BE-006: API 路由

- [ ] `src/app/api/rewrites/direct/generate/route.ts`（POST）：
  - `auth()` + EMPLOYEE 及以上
  - Zod 校验请求体（`DirectGenerateSchema`，含可选 `rewriteId`）
  - 调用 `rewriteService.generateDirect`
- [ ] `src/app/api/rewrites/direct/[rewriteId]/route.ts`（GET）：
  - `auth()` + EMPLOYEE 及以上
  - 调用 `rewriteService.getDirectRewrite`
- [ ] `src/app/api/rewrites/direct/[rewriteId]/versions/[versionId]/route.ts`（PATCH）：
  - `auth()` + EMPLOYEE 及以上
...
- [ ] `src/types/ai-workspace.ts`：新增 `DirectGenerateRewriteInput` 接口（含可选 `rewriteId` 与 `topic` 字段）
- [ ] `src/types/ai-workspace.ts`：`RewriteDTO.workspaceId` 改为 `string | null`，并新增 `mode`、`topic`
