...
- [x] **BE-001**: (P0) 更新 Prisma Schema 与迁移
  ...
  - 详情:
    ...
    5. 新增 `AiDecompositionAnnotation`
    6. 新增 `AiRewriteDraft`
    7. 给 `BenchmarkVideo`、`DouyinVideo` 保持/补齐 `shareUrl`

- [x] **BE-002**: (P0) 改造环境变量与 AI Gateway
  - 文件: `src/lib/env.ts`, `src/lib/env.test.ts`, `src/server/services/ai-gateway.service.ts`
  - 详情:
    1. 支持专用转录运行时配置：`TRANSCRIBE_BASE_URL / TRANSCRIBE_API_KEY / TRANSCRIBE_MODEL_NAME`
    2. 保留系统级步骤绑定
    3. AI Gateway 改为模型池注册表，支持每个实现声明 provider / supportedSteps / requiredEnvKeys
    4. `generateText(step, prompt, implementationKey?)` 成为统一入口
...
- [x] **BE-005**: (P0) 改造转录主链路，把结果落到工作台
  ...
  - 详情:
    ...
    4. 关联当前员工，而不是全局共享
    5. `shareUrl` 缺失时，转录前先尝试爬虫补拉并回写 `BenchmarkVideo.shareUrl`
    - 当前状态:
      1. 工作台转录任务已经按 `shareUrl -> AiWorkspaceTranscript` 落库
      2. 旧 `/api/transcriptions` 链路已移除，转录正式收口为 AI 工作台单轨
      3. 历史 benchmark 视频缺失 `shareUrl` 时，会在发起转录前按 `videoId` 自动补链
...
- [x] **BE-008**: (P0) 实现结构化拆解 CRUD
  ...
  - 详情:
    1. 新增/编辑/删除拆解
    2. 支持任意文本范围
    3. 存储 `function/technique/purpose/effectiveness/argumentRole/note`
    - 当前状态:
...
- [x] **BE-009**: (P0) 实现“解锁编辑会清空拆解与仿写”的原子逻辑
  ...
  - 详情:
    ...
    3. 返回最新工作台
    4. 同一套 reset 逻辑也用于 `转录 / 拆解` 阶段的重新转录
...
- [~] **BE-011**: (P0) 补齐测试
  ...
  - 详情:
    ...
    - 当前状态:
      ...
      3. 本轮定向执行 27 项核心 route/service 测试全部通过
      4. worker 与更完整 UI 集成覆盖仍可后续加强
