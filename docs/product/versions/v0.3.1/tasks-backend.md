...
- `src/server/repositories/transcription.repository.ts`
- `src/lib/env.ts`
...
- [ ] **BE-001**: (P0) 更新 Prisma Schema 与迁移
...
- [ ] **BE-003**: (P0) 完成 AI 配置 Repository / Service / API
...
- [ ] **BE-004**: (P0) 建立 AI 工作台 Repository / Service
...
- [ ] **BE-005**: (P0) 改造转录主链路，把结果落到工作台
  - 文件: `src/server/services/transcription.service.ts`, `src/server/repositories/transcription.repository.ts`, `src/lib/transcription-worker.ts`
  - 详情:
    1. 继续复用异步转录能力
...
- [ ] **BE-009**: (P0) 实现“解锁编辑会清空拆解与仿写”的原子逻辑
