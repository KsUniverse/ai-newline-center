...
- [~] **FE-006**: (P0) 实现转录主文档编辑区与正文画布
  ...
  - 当前状态:
    ...
    3. 已支持在 `转录 / 拆解` 阶段重转录前的 destructive confirm，并在 `仿写` 阶段禁止回退
...
- [~] **FE-010**: (P0) 补齐交互测试与基础校验
  ...
  - 详情:
    1. 工作台布局切换
    2. 解锁编辑提示
    3. 缺少已入库 `shareUrl` 时允许服务端补链后转录
    4. 进入仿写后的缩略卡与三栏位置变化
    5. `转录 / 拆解 / 仿写` 阶段切换与重转录确认
    6. 当前已通过 `pnpm type-check` 和 `pnpm lint`
    7. 路由 / service 测试已补齐核心 AI 主链路
  - 当前状态:
