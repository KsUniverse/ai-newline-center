...
- [ ] **FE-004**: (P0) 用统一 AI 工作台替换旧 benchmark 详情弹框主入口
  - 文件: `src/components/features/benchmarks/benchmark-video-detail-panel.tsx` 及相关入口
  - 详情:
    1. 点击视频后进入 AI 工作台
    2. 旧详情层退化为视频播放辅助层

- [ ] **FE-005**: (P0) 实现工作台初始态三栏布局
  - 文件: `src/components/features/benchmarks/ai-workspace-panel.tsx`（新建）及子组件
  - 详情:
    1. 第一栏视频详情
    2. 第二栏转录主文档
    3. 第三栏拆解区

- [ ] **FE-006**: (P0) 实现转录主文档编辑区
  - 文件: `src/components/features/benchmarks/ai-workspace-transcript-column.tsx` 及子组件
  - 详情:
    1. 展示 AI 原稿和当前稿
    2. 支持修词
    3. 支持语义分段
    4. 支持确认转录稿
    5. 支持解锁编辑
    6. 解锁时通过确认框提示会丢失拆解/仿写

- [ ] **FE-007**: (P0) 实现拆解区
  - 文件: `src/components/features/benchmarks/ai-workspace-decomposition-column.tsx`
  - 详情:
    1. 支持选中文本范围    3. 支持结构化字段编辑
    4. 顶部展示结构地图

- [ ] **FE-008**: (P0) 实现仿写态动画切换
  - 文件: `src/components/features/benchmarks/ai-workspace-panel.tsx`
  - 详情:
    1. 初始态：视频 / 转录 / 拆解
    2. 仿写态：拆解 / 转录 / 仿写
...
- [ ] **FE-009**: (P0) 实现仿写主工作区
  - 文件: `src/components/features/benchmarks/ai-workspace-rewrite-column.tsx`
  - 详情:
    1. 右侧作为主仿写区
    2. 支持人工编辑与草稿保存
    3. 与中间原文形成清晰对照
