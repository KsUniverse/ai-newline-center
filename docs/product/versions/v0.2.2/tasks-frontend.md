...
- [x] **FE-002**: (P0) 创建对标账号主列表页 `/benchmarks`
...
- [x] **FE-003**: (P0) 创建归档列表页 `/benchmarks/archived`
...
- [x] **FE-004**: (P0) 创建对标账号详情页 `/benchmarks/[id]`
...
- [x] **FE-005**: (P0) 创建 `BenchmarkAddDrawer` 组件
...
- [x] **FE-006**: (P0) 创建 `BenchmarkCard` + `BenchmarkCardGrid` 组件
  - 文件:
...
- [x] **FE-007**: (P0) 创建 `BenchmarkDetailHeader` + `BenchmarkEmptyState` 组件
  - 文件:
...
- [x] **FE-008**: (P0) 创建 `BenchmarkVideoList` + `BenchmarkVideoGridCard` 组件
  - 文件:
...
- [x] **FE-009**: (P0) 创建 `src/components/features/benchmarks/index.ts` barrel export
...
### 完成情况
- [x] FE-001: 侧边栏新增 `Target` 图标 + 「对标账号」导航入口，插入「我的账号」之后
- [x] FE-002: 主列表页 `/benchmarks` — `DashboardPageShell` + `BenchmarkCardGrid` + `BenchmarkAddDrawer` + `AlertDialog` 归档确认
- [x] FE-003: 归档列表页 `/benchmarks/archived` — 独立视图，无 Shell，保持与详情页一致的返回导航风格
- [x] FE-004: 详情页 `/benchmarks/[id]` — `BenchmarkDetailHeader` + `BenchmarkVideoList` + `AlertDialog` 归档确认 + 归档后跳转
- [x] FE-005: `BenchmarkAddDrawer` — 复用 `toCreateDouyinAccountPayload`，差异仅在接口路径和错误码映射，通过 `getPreviewErrorMessage` 辅助函数隔离
- [x] FE-006: `BenchmarkCard` + `BenchmarkCardGrid` — 卡片含归档时间展示（archived 模式）和 DropdownMenu 操作入口（作者本人 + 非归档时）
- [x] FE-007: `BenchmarkDetailHeader` + `BenchmarkEmptyState` — DetailHeader 参照 AccountDetailHeader 无同步区域版本；EmptyState 支持 archived 切换文案
- [x] FE-008: `BenchmarkVideoList` + `BenchmarkVideoGridCard` — VideoGridCard 完整复用 hover 播放逻辑，顶右角增加「未拆解」Badge占位
- [x] FE-009: barrel export `index.ts` — 含 3 个页面视图组件 + 7 个业务组件

### 技术决策记录

1. **AlertDialog 异步关闭策略**：`AlertDialogAction` 会在 click 时立即关闭 Dialog（Radix UI 行为）。为保证归档失败时能展示 toast 且不阻塞 UX，采用「先关闭 Dialog，再 fire-and-forget async 调用」模式，取消了 `loading` 状态，改用 `apiClient.del().then().catch()` 链。确认按钮改用 `<Button variant="destructive">` 而非 `<AlertDialogAction>` 以完全控制关闭时机。

2. **页面结构**：归档列表页（FE-003）不使用 `DashboardPageShell`，而用带返回导航的自定义 div 结构，与详情页保持一致（两者均有 Header 区域的返回按钮）。

3. **BenchmarkAddDrawer 错误映射**：将错误码映射逻辑提取为 `getPreviewErrorMessage(error)` 纯函数，与组件渲染逻辑分离，便于 v0.x 版本添加新错误码时维护。

4. **BenchmarkVideoList 的 LIMIT**：`BenchmarkVideoList` 内部固定 `LIMIT = 20`，与父页面 `VIDEOS_PER_PAGE = 20` 保持一致。未将 limit 作为 prop 暴露（与 `VideoList` 不同），因为对标视频列表当前无需动态 limit。

5. **`BenchmarksPageView` / `BenchmarksArchivedPageView` / `BenchmarkDetailPageView`** 均额外从 barrel 导出，以便 page.tsx 稳定导入。

### 问题与提议

1. **`BenchmarkAddDrawer` 与 `AccountAddDrawer` 的结构高度相似**（仅接口路径、文案、错误码不同）。建议后续抽一个共享的 `AddAccountDrawer` 基础组件，通过 `config` props 传入接口路径/文案/错误码映射，减少代码冗余。可在 `docs/architecture/frontend.md` 中补充「可配置 Drawer 模式」规范。

2. **列表页 Header + 分页** 模式在 `BenchmarksPageView` 和 `BenchmarksArchivedPageView` 中基本一致（加载骨架 → 空状态 → 列表 → 分页）。建议提炼为共享的 `CardListSection` 组件，传入 `loading`/`items`/`emptyState`/`renderGrid` 等 props，可被 benchmarks 和未来其他域复用。

3. **`BenchmarkVideoGridCard` 的「未拆解」Badge**：当前为固定文本占位（v0.3.x 占位）。v0.3.x 实现拆解功能时，需将 Badge 替换为动态状态（已拆解/未拆解），可能需要 `DouyinVideoDTO` 扩展字段或独立状态 API。建议在 v0.3.x 技术设计中提前规划。
