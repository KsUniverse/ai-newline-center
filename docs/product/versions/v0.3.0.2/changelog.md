...
- 添加账号时通过 Playwright 打开 www.douyin.com 主站登录弹框，展示二维码供员工扫码
...
- 账号详情页右侧登录状态卡片已移除，仅保留头部状态徽标与“更新登录”入口
- 重登录（`AccountReloginDialog`）强制校验扫码账号与目标账号 `secUserId` 一致，扫错账号不会覆盖登录态
- 收藏同步按账号读取已绑定的 aweme/favorite 原始 Cookie header，透传给 crawler API
