...
import type { FragmentDTO, FragmentScope } from "@/types/fragment";
...
interface ViewpointsTodayListProps {
  scope: FragmentScope;
...
export function ViewpointsTodayList({
  scope,
  currentUserId,
  currentUserRole,
}: ViewpointsTodayListProps) {
  ...
  return (
    <div className="space-y-4">
      <Input
        type="search"
        placeholder={scope === "today" ? "搜索今日观点..." : "搜索观点内容..."}
      ...
      {!initialLoaded ? (
        ...
        <TaskEmptyState
          ...
          tone={scope === "today" ? "default" : "muted"}
        />
      ) : (
        <div className="space-y-3">
          {items.map((item) => {
