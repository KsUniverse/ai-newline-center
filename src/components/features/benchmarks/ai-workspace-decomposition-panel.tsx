  ...
  return (
    <section className="relative flex h-full min-h-0 flex-1 flex-col">
      ...
      <div className="relative flex min-h-0 flex-1 flex-col gap-4 px-4 py-4 sm:px-5">
        {inputMode ? (
          <>
            <div className="space-y-3 rounded-xl border border-border/55 bg-card/80 px-4 py-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-border/45 bg-background/80 text-muted-foreground transition-colors hover:border-primary/20 hover:text-foreground"
                    aria-label="清除选区"
                  >
            ...
            <div className="rounded-xl border border-border/55 bg-background/80 p-4">
              ...
              <textarea
                ...
                rows={10}
                className="mt-4 min-h-0 w-full resize-none rounded-lg border border-border/55 bg-card/82 px-4 py-3 text-sm leading-7 text-foreground outline-none transition-colors placeholder:text-muted-foreground/60 focus:border-primary/30 focus:ring-2 focus:ring-primary/10"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="space-y-3 pr-1">
              {annotations.map((annotation, index) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-xl border border-border/55 bg-card/75 px-4 py-3 text-left transition-all hover:border-primary/14 hover:bg-primary/5 ${
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <div className="mt-3 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-3 overflow-hidden text-sm leading-6 text-foreground/90">
                            ...
                            {quote.collapsed ? (
                              <span className="shrink-0 rounded-md border border-border/45 bg-background/80 px-3 py-0.5 text-[10px] font-medium tracking-[0.18em] text-muted-foreground/85">
                        ...
                        <TooltipContent
                          side="top"
                          align="start"
                          className="max-w-md whitespace-pre-wrap rounded-lg border border-border/55 bg-card/95 px-3 py-2 text-sm leading-6 text-foreground shadow-xl wrap-anywhere"
                        >
              ...
              {annotations.length === 0 ? (
                <div className="rounded-xl border border-dashed border-border/55 bg-background/70 px-4 py-6 text-sm leading-6 text-muted-foreground/80">
                  先在左侧主文档里划词，再保存第一条拆解。
                </div>
              ) : null}
            </div>
