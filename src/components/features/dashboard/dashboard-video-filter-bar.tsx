...
export function DashboardVideoFilterBar({
  ...
  return (
    <div className="rounded-lg border border-border/55 bg-background/70 p-2.5">
      <div className="flex flex-wrap items-center gap-2.5">
        <div className="inline-flex items-center gap-1.5 rounded-md border border-border/45 bg-card/80 px-2.5 py-0.5 text-sm text-muted-foreground">
          <Filter className="h-3.5 w-3.5" />
          <span>筛选条件</span>
        </div>

        <div className="inline-flex rounded-xl border border-border/55 bg-card/80 p-0.5">
        ...
        <Select value={sortBy} onValueChange={(value) => onSortByChange(value as DashboardVideoSortBy)}>
          <SelectTrigger className="h-7.5 w-[9rem] border-border/55 bg-card text-sm">
