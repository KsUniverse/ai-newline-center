...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const { isBringOrder } = bodySchema.parse(body);

    const result = await benchmarkVideoService.updateVideoBringOrder(session.user, id, isBringOrder);
