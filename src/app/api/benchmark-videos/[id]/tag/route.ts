...
const bodySchema = z.object({
  customTag: z.nativeEnum(BenchmarkVideoTag).nullable(),
});
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const { customTag } = bodySchema.parse(body);

    const result = await benchmarkVideoService.updateVideoTag(session.user, id, customTag);
