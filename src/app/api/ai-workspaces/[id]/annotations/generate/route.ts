...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
  ...
  try {
    ...
    const prompt = await aiWorkspaceService.buildGeneratedAnnotationPrompt(id, session.user, body);
    const streamResult = await aiGateway.streamText("DECOMPOSE", prompt);
      ...
      start(controller) {
        void (async () => {
          ...
          try {
            ...
            const streamError = toStreamError(error, "AI 拆解生成失败");
            controller.enqueue(
              encoder.encode(
